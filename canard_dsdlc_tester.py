...
def run_test(msg, msg_type, build_dir):
    ...
    if len(payload) == 0:
        return
    msg_struct = uavcan.transport.CompoundValue(msg, _mode=msg_type)
    print(msg, msg_struct)
    msg_struct._unpack(uavcan.transport.bits_from_bytes(payload))
    print(msg_struct)
    print(uavcan.transport.bits_from_bytes(payload))
    print(msg_struct._pack())
    #pack the struct
    repacked_payload = bytearray.hex(uavcan.transport.bytes_from_bits(msg_struct._pack()))
    print("Packed Struct:", repacked_payload)
    msg_struct._unpack(uavcan.transport.bits_from_bytes(bytearray.fromhex(repacked_payload)))
    print(msg_struct)
    print(msg_struct._pack())
...
if __name__ == '__main__':
    ...
    msg._unpack(uavcan.transport.bits_from_bytes(payload))
    print(msg)
    payload_bits = msg._pack()
    print(bytearray.hex(uavcan.transport.bytes_from_bits(payload_bits)), hexstr_data)
